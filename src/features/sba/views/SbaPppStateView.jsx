...
import { fetchSbaPppState } from "../adapters/sbaPppState";
import {
...
function SbaPppStateView() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const result = await fetchSbaPppState({
  ...
  return (
    <Card className="space-y-6 p-6">
      <div className="space-y-2">
        <h2 className="text-lg font-semibold">SBA PPP — By State</h2>
        <p className="text-sm text-zb-ink-muted">
          Paycheck Protection Program approvals across states.
        </p>
        <p className="text-xs text-zb-ink-muted">
          Uses SBA Open Data resources for PPP FOIA reporting.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-zb-ink-muted">
        ...
        {data && <span>States: {formatNumber(chartData.length)}</span>}
      </div>
      ...
      {status === "success" && data && (
        <SbaBarChart
          title={`States (${metric === "amount" ? "Total $" : "Loan Count"})`}
          data={chartData}
          valueFormatter={
...
export default SbaPppStateView;
